...
def data_augmentation(dataset_name=None, dataset_config=None, path_files=None, gan_general_config=None):
    ...
    for file in glob.glob("gan_generator_*_*.h5"):
        split_file = file.split("_")
        floor_to_analyse.append(split_file[2])
    ...
    for id, flr in enumerate(unique_floors):
        ...
        while (np.shape(y_train_selected)[0] + np.shape(df_new_fake_labels)[0]) <= max_class_val:
            for file in glob.glob("cgan_generator_" + str(flr) + "_*.h5"):
                print(file)
                ...
                latitude = latitude[0][[index_acc_lab]]
                longitude = longitude[0][[index_acc_lab]]
                fake_fingerprints = fake_fingerprints.loc[index_acc_lab, :].copy().reset_index(drop=True)

                # Select realistic fingerprints
                distance_matrix = np.zeros((np.shape(y_train_selected)[0], gan_general_config["num_fake_samples"]))
                ...
                for i in range(0, (np.shape(distance_matrix)[0]) - 1):
                    for j in range(0, (np.shape(distance_matrix)[1]) - 1):
                        ...
                        if distance_matrix[i, j] < 3:
                            if (y_train_selected['FLOOR'].iloc[i] != floor[j]) or (
                                    y_train_selected['BUILDINGID'].iloc[i] != floor[j]):
                                distance_matrix[i, j] = 1000000

                distance_df = pd.DataFrame(distance_matrix)
                filter = ((distance_df < 3) & (distance_df > 0)).any()

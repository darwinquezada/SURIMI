...
def train(X_train, y_train, iteration, g_model, d_model, gan_model, dataset_config=None, gan_general_config=None,
          path_config=None, floor=None, algorithm=None):
    if gan_general_config['train'] == 'True':
        ...
        misc = Misc()
        main_path_save = os.path.join(path_config['saved_model'], dataset_config['name'], algorithm)
        ...
        g_model.save(main_path_save + '/cgan_generator_' + str(gan_general_config['epochs']) + '_' +
                     str(gan_general_config['batch_size']) + '_floor_' + str(floor) + '_iter_' + iteration.zfill(2) +
                     '.h5')


def train_imbalance_classes(X_train, y_train, dataset_config=None, discriminator_config=None, gan_general_config=None,
                            gan_config=None, path_config=None, algorithm=None):
    misc = Misc()
    print(misc.log_msg('WARNING', "----------------- Training GAN ------------------"))
    classes_val = Counter(y_train.values[:, 3])  # 3-Floor, 4-Building
    ...
    for candidate, idx_class in enumerate(list_candidates):
        index_samples = y_train[y_train['FLOOR'] == int(index[idx_class])].index
        X_train_selected = X_train[index_samples, :]
        y_train_selected = y_train[y_train['FLOOR'] == int(index[idx_class])].values
        if samples[idx_class] > 1000:
            segment = np.array_split(X_train_selected, round(np.shape(X_train_selected)[0] / 1000))
            segmenty = np.array_split(y_train_selected, round(np.shape(y_train_selected)[0] / 1000))
            # segment = round(samples[idx_class]/1000)
            count = 1
            for i in range(0, (np.shape(segment)[0]-1)):
                X_train_segment = segment[i]
                X_train_segment = X_train_segment.reshape((X_train_segment.shape[0], X_train_segment.shape[1], 1))
                y_train_segment = segmenty[i][:, 3]  # 2-Floor, 3 Building
                ...
                # train model
                train(X_train_segment, y_train_segment, i, g_model, d_model, gan_model, dataset_config=dataset_config,
            ...
            X_train_segment = X_train_segment.reshape((X_train_segment.shape[0], X_train_segment.shape[1], 1))
            y_train_segment = y_train_selected[:, 3]  # 2-Floor, 3 Building

...
class CNN_LSTM():
    ...
    def floor_model(self):
        ...
        self.fl_model.add(TimeDistributed(Conv1D(filters=32, kernel_size=1, activation='relu',
                                                 padding='same')))
        # self.fl_model.add(TimeDistributed(Conv1D(32, kernel_size=1, padding='same', activation='relu',
    ...
    def position_model(self):
        ...
        self.model_pos.add(TimeDistributed(Conv1D(filters=16, kernel_size=1, activation='elu', padding='same')))
        self.model_pos.add(TimeDistributed(MaxPooling1D(pool_size=1)))
        self.model_pos.add(TimeDistributed(Dropout(0.5)))
        # self.model_pos.add(TimeDistributed(Conv1D(filters=16, kernel_size=1, activation='elu', padding='same')))
        # self.model_pos.add(TimeDistributed(MaxPooling1D(pool_size=1)))
    ...
    def train(self):
        ...
        if (self.position_config['train']).upper() == 'TRUE':
            ...
            # Plot loss vs. val_loss
            # plt.plot(pos_history.history['loss'])
            # plt.plot(pos_history.history['val_loss'])
            # plt.title(self.dataset_config['name'] + ' Model loss - Positioning model')
            # plt.ylabel('Loss')
            # plt.xlabel('Epoch')
            # plt.legend(['Train', 'Validation'], loc='upper left')
            # plt.show()

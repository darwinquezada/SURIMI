...
def define_discriminator(X_train=None, y_train=None, discriminator_config=None):
    ...
    # scale up to radiomap dimensions with linear activation
...
def define_generator(X_train=None, y_train=None):
    ...
    # merge radiomap gen and label input
...
def define_gan(g_model, d_model, gan_config=None):
    ...
    # get radiomap output from the generator model
    gen_output = g_model.output
    # connect radiomap output and label input from generator as inputs to discriminator
...
def generate_fake_samples(generator, latent_dim, n_samples, n_classes=1):
    ...
    # predict outputs
    radiomap = generator.predict([z_input, labels_input])
    # create class labels
    y = zeros((n_samples, 1))
    return [radiomap, labels_input], y
...
def train(X_train, y_train, iteration, g_model, d_model, gan_model, dataset_config=None, gan_general_config=None,
          path_config=None, floor=None, building=None):
    if gan_general_config['train'] == 'True':
        ...
        if floor is None:
            floor = 0
        print(misc.log_msg('WARNING', "-------Building " + str(building) + " Floor " + str(floor) +
                           "- Iteration " + iteration.zfill(2) + " -------"))
        ...
        for i in range(n_epochs):
            ...
            for j in range(bat_per_epo):
                ...
                X_real, labels_real, y_real = data_selection(X_train=X_train, y_train=y_train,
                                                             num_samples=int(np.shape(X_train)[0]/n_epochs))
                # X_real = X_train
                # labels_real = y_train
                ...
                [X_fake, labels], y_fake = generate_fake_samples(g_model, latent_dim, n_samples, n_classes)
                # update discriminator model weights
        ...
        g_model.save(main_path_save + '/cgan_generator_' + str(gan_general_config['epochs']) + '_' +
                     str(gan_general_config['batch_size']) + '_b_' + str(building) + "_f_" + str(floor) + '_i_' +
...
def train_imbalance_classes(X_train, y_train, dataset_config=None, discriminator_config=None, gan_general_config=None,
    ...
    # Select information by building
    for bld_idx, bld in enumerate(np.unique(y_train['BUILDINGID'])):
        bld_index_samples = y_train[y_train['BUILDINGID'] == int(bld)].index
        X_train_bld_selected = X_train[bld_index_samples, :]
        y_train_bld_selected = y_train[y_train['BUILDINGID'] == int(bld)].reset_index(drop=True)

        # Select information by floor
        classes_val = Counter(y_train_bld_selected.values[:, 3])  # 3-Floor, 4-Building
        ...
        for candidate, idx_class in enumerate(list_candidates):
            index_samples = y_train_bld_selected[y_train_bld_selected['FLOOR'] == int(index[idx_class])].index
            X_train_selected = X_train_bld_selected[index_samples, :]
            y_train_selected = y_train_bld_selected[y_train_bld_selected['FLOOR'] == int(index[idx_class])].values
            ...
            train(X_train_segment, y_train_segment, 0, g_model, d_model, gan_model, dataset_config=dataset_config,
                  gan_general_config=gan_general_config, path_config=path_config, floor=int(index[idx_class]),
                  building=int(bld))
